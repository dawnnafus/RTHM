...
function(input, output) {
      ...
      output$pollutantSelect <- renderUI({
        selectInput("pollutant_var", "Pollutant", 
                    ...
                    selected = pollutantVariables[1])
      })
      
      output$plot <- renderPlot({
        final_df %>%
          select(input$health_var, input$pollutant_var, day) %>%
          filter(!is.na(input$health_var) | !is.na(input$pollutant_var))%>%
          group_by(day) %>%
          summarize(mean_health_var = mean(input$health_var, na.rm=T),
                    mean_pollutant_var = mean(input$pollutant_var, na.rm=T)) %>% 
          ungroup %>%
