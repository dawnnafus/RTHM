...
```{r create time column, message=F, warning=F}
# 5/13 0:00 - 7/14 0:00
...
q <- df %>% 
  select(day, id, blood_oxygen, sulfur_dioxide) %>% 
  filter(!is.na(blood_oxygen) | !is.na(sulfur_dioxide)) %>% 
  arrange(day) %>% spread(id,id)

qs<-q %>% 
  group_by(day) %>%
  summarize(cor = cor(blood_oxygen, sulfur_dioxide)) %>% 
  ungroup() #%>%
  summarize(mean = mean(cor))

# Need to spread by id, but need fb to have id's like paco

q %>% filter(heart_rate > 50 | is.na(heart_rate)) %>% ggplot() + 
  geom_point(aes(x = day, y = sulfur_dioxide, color = feed)) + 
  geom_point(aes(x = day, y = heart_rate))

# Missing data from both feeds
q %>% spread() %>% select(heart_rate, sulfur_dioxide) %>% boot::corr()
