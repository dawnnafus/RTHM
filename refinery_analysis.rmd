...
library(rgdal)
cc <- readOGR(dsn ="../refinery_data/location/BaseMapGrid", layer = "GRD_PWD_BaseMap_0106") #read shapefile
summary(cc)
cc_wgs84 <- spTransform(cc, CRS("+proj=longlat +datum=WGS84 +no_defs")) # convert to lat long
summary(cc_wgs84)
coordinates(cc_wgs84)
plot(cc_wgs84)
cc_data <- as.owin(cc_wgs84) #convert to dataset for analysis
```
...
library(maps)
library(RgoogleMaps)
g <- ggplot(cc.utm, aes(x=long, y=lat)) +   
...
bgMap = get_map(as.vector(bbox(cc.utm)), source = "google", zoom = 7) # useless without zoom level
plot(spTransform(cc.utm, merc), bgMap = bgMap, pch = 16, cex = .5)
merc = CRS("+init=epsg:3857")

center = apply(coordinates(cc.utm), 2, mean)[2:1]
g = GetMap(center=center, zoom=7) # google
par(mar = rep(0,4)) # fill full device
spplot(spTransform(cc.utm, merc), colorkey = TRUE,
