...
### Individual Data
```{r make individual data, message=F, warning=F}
# Collect addresses
...
### Air Quality Data
```{r create air quality dataset, message=F, warning=F}
# Air Quality data
...
# Blood oxygen and sulfur dioxide
temp_bosd<- ind_data %>%
  #filter(heart_rate > 30 & heart_rate < 200) %>% # May want to keep the same filter
  #filter(age > 50 & sex == "male") %>%
  bind_rows(air_quality, .id = NULL) %>%
  select(heart_rate, sulfur_window, day) %>%
  filter(!is.na(blood_oxygen) | !is.na(sulfur_window)) %>% 
...
# moran's i - need a universe of points
# https://stackoverflow.com/questions/15711013/spatial-autocorrelation-analysis-in-r?rq=1
