---
title: "Linking Health Outcomes to Refinery Emissions"
author: "Niklas Lollo"
output: 
  html_document:
    toc: true
...
```{r filter your data, message=F, warning=F, echo=F}
...
Methane and sulfur dioxide readings collected daily by [Fenceline.org](fenceline.org)
...
correlation_text <- function(ind_data = ind_data_filtered, air_data = air_quality, 
...
correlation_num <- function(ind_data = ind_data_filtered, air_data = air_quality, 
                            var_1, var_2, var_time = day) {
    var_1 <- enquo(var_1)
    var_2 <- enquo(var_2)
    var_time <- enquo(var_time)
...
ind_data_hr <- ind_data_filtered %>%
  filter(heart_rate > 30 & heart_rate < 200)
# No filter
correlation_text(
  ind_data = ind_data_hr,
  var_1 = heart_rate,
  var_2 = sulfur_dioxide_exposure_window)
...
## T-Tests^[Welch two-sample t-test] for Statistical Significance
...
mean_pop_hr <- mean(ind_data_hr$heart_rate, na.rm=T)
mean_samp_hr <- mean(temp_hrm$mean_hr, na.rm=T)
  
t_hrm <- t.test(x = temp_hrm$mean_hr, y = ind_data_hr$heart_rate)

# t.test(temp_hrm$mean_hr, mu = mean_pop_hr)
# Both work fine but above one corrects for difference variances

# Heart rate and sulfur dioxide
temp_hrsd<- ind_data_hr %>%
  bind_rows(air_quality, .id = NULL) %>%
  select(heart_rate, sulfur_dioxide_exposure_window, day) %>%
  filter(!is.na(heart_rate) | !is.na(sulfur_dioxide_exposure_window)) %>% 
  group_by(day) %>%
...
t_hrsd <- t.test(x = temp_hrsd$mean_hr, y = ind_data_hr$heart_rate)
#t.test(temp_hrsd$mean_hr, mu = mean_pop_hr)

# Blood oxygen and sulfur dioxide
temp_bosd<- ind_data_filtered %>%
  bind_rows(air_quality, .id = NULL) %>%
  select(blood_oxygen, sulfur_dioxide_exposure_window, day) %>%
  filter(!is.na(blood_oxygen) | !is.na(sulfur_dioxide_exposure_window)) %>% 
  group_by(day) %>%
...
mean_bo<- mean(ind_data_filtered$blood_oxygen, na.rm=T)
t_bosd <- t.test(x = temp_bosd$mean_bo, y = ind_data_filtered$blood_oxygen)
#t.test(temp_bosd$mean_bo, mu = mean_bo)
