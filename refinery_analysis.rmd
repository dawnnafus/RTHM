...
timeframe <- function(date_begin = date_begin, 
                      date_end = date_end){
  exp_begin = as.POSIXct(date_begin, tz="GMT", format="%Y-%m-%d %H")
  exp_end = as.POSIXct(date_end, tz="GMT", format="%Y-%m-%d %H")
...
# Loads pre-saved file from website
...
### Spatial analysis
```{r spatial autocorrelation, warning=F, message=F}
# For binary data, use the join-count index

# First, determine which points are near
# Select only the spatial points
points <- spatial_df %>% select(longitude, latitude)
# Turn that into special dataframe
...
moran.mc(spatial_df$heart_rate, 
         spatial_listw, 
         nsim=99,
         zero.policy = T, 
         na.action = na.omit)
