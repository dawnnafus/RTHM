...
ind_data_filtered %>%
  ...
  theme_fair_tech + 
  labs(x=expression(paste("Sulfur Dioxide ( ", degree ~ F, " )")), 
       y=expression(paste("Blood Oxygen ( ", degree ~ F, " )"))) +
...
ind_data_hr %>%
  ...
  geom_smooth(method = "lm", se=F) + 
  theme_fair_tech + 
  labs(x=expression(paste("Methane ( ", degree ~ F, " )")), 
       y=expression(paste("Heart Rate ( ", BPM, " )"))) +
...
cat("Heart Rate and Methane")
t.test(x = temp_hrm$mean_hr, y = ind_data_hr$heart_rate)
# t.test(temp_hrm$mean_hr, mu = mean_pop_hr)
# Both work fine but above one corrects for difference variances
# Here we see methane above its mean value is associated with a heart rate decrease of about 4 points 
...
cat("Heart Rate and Sulfur Dioxide")
t.test(x = temp_hrsd$mean_hr, y = ind_data_hr$heart_rate)
...
cat("Blood Oxygen and Sulfur Dioxide")
t.test(x = temp_bosd$mean_bo, y = ind_data_filtered$blood_oxygen)
