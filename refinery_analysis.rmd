...
# List of R packages
required_pkg <- c("tidyverse","maptools", "rgdal", "spatstat", "ggmap", "geosphere", "leaflet")
...
m3_distance_meters <- distm (m3_locale, refinery, fun = distHaversine)
m5_distance_meters <- distm (m5_locale, refinery, fun = distHaversine)
m7_distance_meters <- distm (m7_locale, refinery, fun = distHaversine)
m9_distance_meters <- distm (m9_locale, refinery, fun = distHaversine)
m10_distance_meters <- distm (m10_locale, refinery, fun = distHaversine)
m11_distance_meters <- distm (m11_locale, refinery, fun = distHaversine)
m16_distance_meters <- distm (m16_locale, refinery, fun = distHaversine)
...
demographics <- data_frame(
  ...
  home_dist_to_refinery = c(m3_distance_meters, m5_distance_meters,
                            m7_distance_meters, m9_distance_meters,
                            m10_distance_meters, m11_distance_meters,
                            m16_distance_meters),
...
total_df <- individual_data %>%
  ...
  bind_rows(air_quality, .id = NULL) %>% # add air_quality data
  mutate(id = as.factor(id))

binding_df <- total_df %>%
  select(-c(methane, sulfur_dioxide))
...
r + 
  geom_point(aes(x= demographics$home_long[1], y=demographics$home_lat[1])) +
  geom_point(aes(x= demographics$home_long[2], y=demographics$home_lat[2])) +
  geom_point(aes(x= demographics$home_long[3], y=demographics$home_lat[3])) +
  geom_point(aes(x= demographics$home_long[4], y=demographics$home_lat[4])) +
  geom_point(aes(x= demographics$home_long[5], y=demographics$home_lat[5])) +
  geom_point(aes(x= demographics$home_long[6], y=demographics$home_lat[6])) +
  geom_point(aes(x= demographics$home_long[7], y=demographics$home_lat[7])) + 
  geom_point(aes(x= refinery[1], y= refinery[2], color = "black"))
...
some_coords <- coordinates(object = spatial_df, value = c("home_long", "home_lat"))
# don't have lat_long data for air_pollution. Need to consider.
my_spatial <- SpatialPointsDataFrame(coords = c(spatial_df$home_long,
                                                spatial_df$home_lat),
