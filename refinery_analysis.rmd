...
paco_hourly <- paco %>%
  ...
  group_by(person,
    # Creates new variables
    day = as.POSIXct(cut(t, breaks='day')),
    hour= format(as.POSIXct(cut(t, breaks='hour')), '%H')) %>%
    # Average
...
# See if day is the same
paco_hourly <- paco_hourly %>% 
  group_by(person) %>%
  mutate(
    subtract = ifelse(day == lag(day), T, F)
    # Now you can search back to find the TRUE's in a row which will give you all the same day
  )



movingavg <- function(the_data, person = person, column, n = 5){
  require(dplyr)
  the_data %>% 
    group_by(person) %>% 
    arrange(day, hour) %>%
  ...
  result = rep(NA, nrow(column))
  for (i in 1:nrow(column)){
    if (i < n){
      next
     }
        result[i] = mean(the_data[(i - n + 1):i])
    }
  return(result)
}
...
paco_hourly$five_blood <- movingavg(paco_hourly, 5)
```
...
paco_hourly <- paco_hourly %>% 
  group_by(person) %>% 
  arrange(day, hour) %>%
  mutate(five_hour = roll_meanr(blood_oxygen,
                            n=5L,
                            fill = NA,
                            na.rm=T,
                            normalize=FALSE)) %>% ungroup
summary(paco_hourly$five_hour)
