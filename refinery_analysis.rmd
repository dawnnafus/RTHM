...
```{r regression, warning=F, message=F, echo=F}
# Health outcome
summary(lm(data = spatial_df, heart_rate ~ methane + sex + age, na.action = na.omit))
summary(lm(data = spatial_df, heart_rate ~ sulfur_dioxide + sex + age, na.action = na.omit))
summary(lm(data = spatial_df, blood_oxygen ~ sulfur_dioxide + sex + age, na.action = na.omit))
# Really need to examine whether heart rate is elevated or blood oxygen is lower than a certain threshold. Also, need to think about what is the dose-response curve expected.
