...
server <- function(input, output) {
  ...
  output$pollutantSelect <- renderUI({
    selectInput("pollutant_var", "Pollutant", 
                choices = c("sulfur_dioxide_exposure_window",
                            "methane"),
                selected = "methane")
  })
  ...
  health <- reactive({
      ...
      final_df %>%
        group_by(day) %>%
        summarize(health = mean(UQ(input$health_var), na.rm=T))
  })
  ...
  pollutant <- reactive({
      ...
      final_df %>%
        group_by(day) %>%
        summarize(pollutant = mean(vars(!!input$pollutant_var), na.rm=T))
  })
  
  output$type <- renderPrint({ 
    typeof(pollutant()$pollutant)
    pollutant()$pollutant
    })
  
  output$view <- renderTable({ pollutant() })
  
  output$plot1 <- renderPlot({
    ggplot() +
      geom_point(aes_string(x = pollutant()$pollutant, y = health()$health))
  })
